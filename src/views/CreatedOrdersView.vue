...
<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <div class="p-6 bg-gray-50">
            ...
            <div class="space-y-3">
              <CreatedOrderItem v-for="orderItem in orderStore.getOrderItems(order)"
                                :key="orderItem.id"
                                :order="order" :order-item="orderItem"/>
            </div>
          </div>
          <!-- Footer do Pedido (Ações) -->
          <div class="px-6 py-4 bg-white border-t border-gray-100">
            <div class="flex gap-3 justify-end">
              <button class="px-4 py-2 text-sm text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                Detalhes
              </button>
              <button class="px-4 py-2 text-sm text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition">
                Atualizar Status
              </button>
            </div>
          </div>
      ...
      <!-- Empty State -->
      <div v-if="orderStore.orders.length === 0" class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
