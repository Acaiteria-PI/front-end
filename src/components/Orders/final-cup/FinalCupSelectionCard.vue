...
<template>
  <div
    :class="[
      ...
      selectedCup === cup.id ? 'border-rose-900 bg-rose-50' : 'border-gray-100',
    ]"
  >
    ...
    <div class="space-y-3">
      ...
      <div class="pt-3 border-t border-gray-100 space-y-2">
        <div class="flex justify-between text-sm">
          <span class="text-gray-600">Recipiente:</span>
          <span class="font-medium text-gray-800">{{ cup.recipient_data.title }}</span>
        </div>
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-1">
            <span
              v-for="(ingredient_data, index) in cup.ingredient_data"
              :key="index"
        ...
        <div class="pt-2">
          <p class="text-2xl font-bold text-blue-600">R$ {{ cup.price.replace(".", ",") }}</p>
        </div>
      </div>
    </div>
  </div>
