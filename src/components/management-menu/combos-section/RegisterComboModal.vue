...
<template>
  ...
  >
    ...
    <form
      @submit.prevent="$emit(mode === 'create' ? 'createCombo' : 'editCombo')"
    ...
    >
      <section class="w-full grid grid-cols-1 gap-4">
        ...
        <section v-if="finalCupStore.finalCups.length > 0" class="gap-1 flex flex-col col-span-2">
        ...
        <section v-else class="gap-1 flex flex-col col-span-2">
          <label for="finalCup">Produtos</label>
