...
function getTotalPrice() {
  const recipientPrice =
    recipientStore.recipients.find((r) => r.id === customCupStore.newCustomCup.recipient) || 0
...
const getRecipientSize = () => {
  const recipient = recipientStore.recipients.find(
    (r) => r.id === customCupStore.newCustomCup.recipient
  )
  return recipient ? recipient.quantity_ml : 'Nenhum'
}
